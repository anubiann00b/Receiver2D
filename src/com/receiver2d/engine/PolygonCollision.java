	...
	public static Vector2D[] checkCollide(Vector2D[] polyA, Vector2D[] polyB) {
		...
		for (int i=0; i<polyA.length; i++) {
			...
			for (int j=0; j<polyB.length; j++) {
				...
				// calculations				
				try {
					/* we must have a try/catch block for potential division
					 * by zero
					 */
					float m_a = (a1.y - a2.y)/(a1.x - a2.x); //slope, vertex a
					float m_b = (b1.y - b2.y)/(b1.x - b2.x); //slope, vertex b
					
					// TODO: check for lines where slope is undefined
					
					float x = (m_a*a1.x-m_b*b1.x + a1.y+a1.x) / (m_a - m_b);
