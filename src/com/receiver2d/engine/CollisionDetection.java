...
public class CollisionDetection {
	 ...
	 */
	public static Vector2f linearIntersectionPoint(Vector2f[] ln1,
			Vector2f[] ln2) {
	 ...
	 */
	public static Vector2f[] polygonalIntersectionPoints(Vector2f[] polyA,
			Vector2f[] polyB) {
		...
		for (int i = 0; i < polyA.length; i++)
			for (int j = 0; j < polyB.length; j++) {
				...
				Vector2f pnt = linearIntersectionPoint(
						new Vector2f[] { a1, a2 }, new Vector2f[] { b1, b2 });
				if (pnt != null)
					results.add(pnt);
			}
		if (results.size() == 0)
			...
			return results.toArray(new Vector2f[results.size()]);
	}
	 ...
	 */
	public boolean checkCollides(Vector2f[] poly1, Vector2f[] poly2) {
