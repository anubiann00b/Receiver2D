...
/**
 * In this class we abstract all drawing functions. We should be able to do, for example, Renderer.drawQuad(Vector2D a, Vector2D b);
 */
public class Renderer {
	...
	/**
	 * Performs a rendering update, in which the Display is cleared and new things are drawn on screen.
	 * 
	 * @return Whether or not the next rendering update is available (based on if the user prompted a display close event). If false, then the next render is prevented and the display (along with the rest of the program) quits.
	 */
